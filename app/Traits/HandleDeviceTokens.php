...
{
    protected function getUsersTokens($tripId, $stationId, $users, $exclude)
    {
        $tokens = User::select('device_id');

        if ($users) {
            $tokens = $tokens->whereIn('id', $users);
        } else {
            $tokens = $tokens->Join('business_trip_users', 'business_trip_users.user_id', '=', 'users.id');
